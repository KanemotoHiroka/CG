...
def intersect_triangle(PE, E, P1, P2, P3):
    ...
    E_norm = len(E)## Calculate the norm of E ##

    if E_norm != 0:
        E = E / E_norm
        N = np.cross(P1-P2, P2-P3)
        t = -(np.dot((PE-P1),N)/np.dot(E,N))## Calculate t to find the intersection point between the ray and the plane ##
        ...
        if t > 0:
            ...
            if np.all(N1)==np.all(N2) and np.all(N2)==np.all(N3) and np.all(N1)== np.all(N3): ## Judge whether the direction of the cross product N1, N2 and N3 is same ##
                intersect_point = P
